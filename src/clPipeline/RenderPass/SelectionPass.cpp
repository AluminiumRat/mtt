...
SelectionPass::SelectionPass( VkFormat colorBufferFormat,
  ...
  _depthBufferLayout(depthBufferLayout)
{
  ...
  subpasses[0].stages.push_back(selectionStage);

  setup(dependencies, attachments, 0, subpasses);
}

Ref<FrameBuffer> SelectionPass::createFrameBuffer(ImageView& colorTarget,
                                                  ImageView& depthMap)
{
